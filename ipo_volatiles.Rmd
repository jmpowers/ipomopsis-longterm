...
```{r quantslopes, eval=FALSE}
quant.slopes <- set_names(rep(0,ncol(quant.all)), colnames(quant.all))
#TODO get this working, already dropped "(3E)-4,8-dimethylnona-1,3,7-triene" from quant.all
for(compvol in colnames(quant.all)) {
  print(compvol)
  nozeros <- bind_cols(select(qual.all,qual=compvol), select(quant.all,quant=compvol)) %>% 
...
chemsf$filter_final <- with(chemsf, filter_RT=="OK" & RT.var < 2 & # variance of retention time < 2 min
                              ...
                              name==c("indole"))#manually included - not detected by peak picker
...
New compounds to investigate that correlate with known floral volatiles:

* methyl 2-methylbutanoate (fatty acid ester, clusters with a-pinene)
* (3E,5E)-2,6-dimethylocta-3,5,7-trien-2-ol (monoterpene alcohol, clusters with pseudoionone)
* 7-epi-trans-sesquisabinene hydrate (C15, clusters with evening volatiles, but 4+ retention times)

New but cluster with contaminants and have multiple retention times:
...
pheatmap(cor(sqrt(qual.all.cut)), scale="none", 
         ...
         annotation_colors=list(status=set_names(brewer.pal(4, "Set1"), levels(chems.cut$status))),
