...
## Compare quant and bouquet compounds
...
meta <- metadata.df[rownames(qual),] %>% 
  mutate(across(c(year_run, site, time), factor))
```
...
```{r quant_heatmap, dev="png", dev.args=list(),fig.width=10, fig.height=8}
pal <- with(meta, list(
  year_run = set_names(brewer.pal(nlevels(year_run), "Set2"), levels(year_run)),
...
pheatmap(as.matrix(t(quant))^(1/4), 
         ...
         annotation_col = meta %>% select(year_run, site, time) %>% mutate(across(everything(), factor)),
...
```{r quant_nmds}
nmds <- metaMDS(decostand(quant, "hellinger"), autotransform = F, trymax=1)

plot(nmds, type="n")
points(nmds, display="sites", col = pal$time[meta$time])
legend("topleft", levels(meta$time), fill=pal$time)
text(nmds, display="species")

plot(nmds, type="n")
points(nmds, display="sites", col = pal$site[meta$site], pch=c(1,19)[as.integer(meta$time)])
legend("topleft", levels(meta$site), fill=pal$site)

plot(nmds, type="n")
points(nmds, display="sites", col = pal$year_run[meta$year_run], pch=c(1,19)[as.integer(meta$time)])
legend("topleft", levels(meta$year_run), fill=pal$year_run)
```
