...
cap.ambi.df <- fortify(cap.ambi) %>% left_join(mutate(meta.all.noleaf, score="sites"), by=c(label="FileName", "score"))

zoom <- 40
cap_threshold <- 3
ggplot(filter(cap.ambi.df, score=="sites"), aes(x=CAP1, y=MDS1, label=greekify(label))) +
  theme_classic() + theme(legend.position = "top") + 
  geom_point(aes(shape=type, color=type)) + scale_shape_manual(values=c(19,1))+
...
contams <- c("decan-2-one", "Nonanoic acid", "Decanal", "Caprolactam")
filter(cap.ambi.df, score=="species") %>% 
  mutate(verdict = if_else(label %in% colnames(quant.all), if_else(label %in% contams, "contam", "keep"), "unknown")) %>% 
ggplot(aes(x=CAP1, y=MDS1, label=greekify(label), color=verdict)) +
  theme_classic() + theme(legend.position = "top") +
  geom_vline(xintercept = c(cap_threshold,cap_threshold), linetype=2)+ geom_vline(xintercept=0)+
  geom_hline(yintercept=-25, linetype=2)+ geom_hline(yintercept=0)+
  geom_point()+  geom_smooth()

ggplot(filter(cap.ambi.df, score=="sites"), aes(y=-CAP1, x=type)) + geom_boxplot()
```
