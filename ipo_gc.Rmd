...
filemeta <- read_csv("data/volatiles/Ipomopsis long-term volatiles - ipo_meta.csv") %>% 
  mutate(type = fct_recode(type, floral="sample"))
filemeta %>% count(site, date, time)

file.metadata <- filemeta %>% select(-index, -time) %>% rename(Filename2 = sample, notes_file = notes, plant = vial) %>% 
